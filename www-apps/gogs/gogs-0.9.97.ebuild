...
src_prepare() {
	...
	sed -i \
		...
		-e "s:^PATH = data/gogs.db:PATH = ${EPREFIX}${GOGS_DIR}/database/${PN}.db:" \
...
src_install() {
	...
	insinto ${GOGS_DIR}/custom/conf
	insopts -m640
	doins conf/app.ini

	# Install HTTPS certs
	if use ssl; then
		keepdir ${GOGS_DIR}/custom/https
	fi

	# Fix permissions of config files
	keepdir ${GOGS_DIR}/custom
	fowners -R ${USER_NAME}:${USER_NAME} ${GOGS_DIR}/custom
